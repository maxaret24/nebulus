...
async def savenote(client,message: Message):
    if not message.reply_to_message:
        return await message.reply_text('`Reply to a message to save it`','md')
    splitted = message.command
    if len(splitted) != 2:
        return await message.reply_text('`Provide a notename to save it`','md')
    ...
    await message.reply_text(f'**Saved note:** `{notename}`','md')
...
async def getnotes(client,message: Message):
    ...
    if not notes:
        return await message.reply_text('`There are no saved notes`','md')
...
async def getnote(client: Client,message: Message):
    splitted = message.command
    if len(splitted) != 2:
        return await message.reply_text('`Please provide a Notename`','md')
    notename = splitted[1].lower().strip()
    data = await get_a_note(notename)
    if not data:
        return await message.reply_text(f'**Note** `{notename}` **not found**','md')
    ds = data.split(':')
    tpe = ds[0] # note type
    meta = ':'.join(ds[1:])
...
async def delete_note(c,m: Message):
    if len(m.command) != 2:
        return await m.reply_text('`Please provide a notename`','md')
    notename = m.command[1]
    resp = await del_note(notename)
    if resp:
        await m.reply_text(f'**Note** `{notename}` **deleted**','md')
    else:
        await m.reply_text(f'**Note** `{notename}` **doesn\'t exist**','md')